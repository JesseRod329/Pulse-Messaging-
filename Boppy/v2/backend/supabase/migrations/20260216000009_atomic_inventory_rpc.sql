create or replace function public.reserve_order_inventory_atomic(
  p_order_id uuid,
  p_actor_id uuid
)
returns jsonb
...
begin
  ...
  v_reason := format('order_reserved:%s', p_order_id);
  ...
  loop
    ...
    if v_next < 0 then
      raise exception 'insufficient_stock:variant:%', v_variant_row.id;
    end if;
  ...
  loop
    ...
    if v_next < 0 then
      raise exception 'insufficient_stock:item:%', v_item_row.id;
    end if;
